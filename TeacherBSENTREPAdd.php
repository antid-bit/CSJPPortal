...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
    ...
    $course_id = 5;  // BSENTREP course_id
    ...
    if (empty($message)) {
        ...
        $check_stmt->bind_param("isi", $subject_id, $day_of_week, $course_id);  // fixed 'day_of_week' as string (s)
        $check_stmt->bind_param("iss", $subject_id, $day_of_week, $course_id);  // Correct bind_param types: i (int), s (string), i (int)
        $check_stmt->execute();
        ...
        if ($check_result->num_rows > 0) {
            ...
            echo "<script>
                    setTimeout(function() {
                        window.location.href = '/CSJPPortal/TeacherBSENTREPSchedule.php';
                    }, 2000);
                  </script>";
        } else {
...
<html>
<head>
    <title>Add New Schedule - BSENTREP</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-100 p-8">
    <h1 class="text-3xl font-bold mb-6">Add New Schedule Entry (BSENTREP)</h1>
    ...
    <form method="POST" class="max-w-lg bg-white p-6 rounded shadow-md">
        ...
        <div class="flex justify-between">
            <a href="/CSJPPortal/TeacherBSENTREPSchedule.php" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">Cancel</a>
