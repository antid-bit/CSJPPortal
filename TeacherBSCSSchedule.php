...
// Fetch BSCS (`course_id = 1`) schedule
...
<body class="bg-gray-100 font-sans leading-normal tracking-normal">
    ...
    <div class="flex-1 p-8 overflow-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold">📅 Weekly Class Schedule (BSCS)</h1>
            <a href="/CSJPPortal/TeacherBSCSAdd.php" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">
                ➕ Add Schedule
            </a>
        </div>
                    ...
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr class="<?= ($row['day_of_week'] === $today) ? 'bg-yellow-100 font-semibold' : 'hover:bg-gray-50' ?>">
                            ...
                            <td class="py-3 px-4 border space-x-2">
                                <form method="POST" action="/CSJPPortal/TeacherBSCSDelete.php" class="inline">
                                    <input type="hidden" name="schedule_id" value="<?= $row['schedule_id'] ?>">
                                    <form method="POST" action="TeacherBSCSDelete.php" onsubmit="return confirm('Are you sure you want to delete this schedule?')" class="inline">
    <input type="hidden" name="schedule_id" value="<?= $row['schedule_id'] ?>">
